...
```bash
bash ../hpc4-docker-vpn/scripts/check_easyconnect_proxy_remote.sh
bash scripts/hpc4_login_via_vpn_remote.sh 'hostname; whoami; pwd'
bash scripts/refresh_npu_pool_via_hpc4_remote.sh 670 ./npu_pool_summary.json
cat ./npu_pool_summary.json
bash scripts/connect_npu_from_summary_remote.sh 1 ./npu_pool_summary.json
```
...
- In the current setup, the SOCKS5 proxy and nested SSH steps run on `ubuntu@43.134.118.168`.
