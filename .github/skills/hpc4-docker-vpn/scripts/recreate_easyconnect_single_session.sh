...
image="$(sudo docker inspect "${container_name}" --format '{{.Config.Image}}')"
env_lines="$(sudo docker inspect "${container_name}" --format '{{range .Config.Env}}{{println .}}{{end}}')"
ec_ver="$(printf '%s\n' "${env_lines}" | sed -n 's/^EC_VER=//p' | head -n1)"
cli_opts="$(printf '%s\n' "${env_lines}" | sed -n 's/^CLI_OPTS=//p' | head -n1)"
root_mount_source="$(sudo docker inspect "${container_name}" --format '{{range .Mounts}}{{if eq .Destination "/root"}}{{println .Source}}{{end}}{{end}}' | head -n1)"
logs_mount_source="$(sudo docker inspect "${container_name}" --format '{{range .Mounts}}{{if eq .Destination "/usr/share/sangfor/EasyConnect/resources/logs"}}{{println .Source}}{{end}}{{end}}' | head -n1)"
