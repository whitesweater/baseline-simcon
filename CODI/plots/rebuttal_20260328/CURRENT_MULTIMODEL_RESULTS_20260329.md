...
- `cot-sft` 当前只有 `llama3-3b` / `qwen3-4b` 的 GSM8K 可以直接汇报；`qwen3-1.7b` 已经训练到 `checkpoint_25`，但 batch multi-dataset eval 失败
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `cot-sft` | `llama3-3b` | `Coconut` | `train complete (checkpoint_25 present)` | `gsm8k only; all-eval interrupted` | `checkpoint_25` | `72.40%` | `-` | `-` | `-` | `-` | `-` | `-` | `checkpoint_25` 的 batch all-eval 先跑出 GSM8K `955/1319`，随后在 `gsm_hard` 因数据类型问题中断 |
