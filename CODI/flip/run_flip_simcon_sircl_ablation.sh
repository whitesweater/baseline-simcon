#!/bin/bash
# =============================================================================
# flip 数据集 simcon_sircl 模型 - 灵活训练/测试脚本
# =============================================================================
# 支持任意 trajectory_loss_factor 值的训练和测试
#
# 用法:
#   ./run_flip_simcon_sircl_ablation.sh train -f 0.25     # 训练 factor=0.25
#   ./run_flip_simcon_sircl_ablation.sh test -f 0.1       # 测试 factor=0.1
#   ./run_flip_simcon_sircl_ablation.sh all -f 0.25       # 训练+测试
#   ./run_flip_simcon_sircl_ablation.sh list              # 列出所有已有模型
#   ./run_flip_simcon_sircl_ablation.sh test -f baseline  # 测试 baseline
# =============================================================================
...
# 基础配置
RESULT_DIR="${CODI_RESULT_DIR}/flip_ablation"
TRAINED_DIR="${CODI_SAVE_DIR}"
BASELINE_NAME="filp_codi_base"
BASELINE_PATH="${TRAINED_DIR}/filp_codi_base"
...
get_model_info() {
    ...
    case "$factor" in
        "baseline")
            MODEL_NAME="filp_codi_base"
            MODEL_PATH="${BASELINE_PATH}"
            ;;
        "0.1")
            # 特殊处理：已有的 0.1 模型目录名不同
            MODEL_NAME="flip_simcon_sircl"
            MODEL_PATH="${TRAINED_DIR}/flip_simcon_sircl"
            ;;
        *)
            # 通用命名：将小数点替换为空，如 0.05 -> factor005, 0.25 -> factor025
            local factor_suffix=$(echo "$factor" | sed 's/0\.//' | sed 's/\.//')
            MODEL_NAME="filp_simcon_sircl_factor${factor_suffix}"
...
list_models() {
    ...
    for dir in "${TRAINED_DIR}"/filp_simcon_sircl* "${TRAINED_DIR}"/flip_simcon_sircl*; do
        if [[ -d "$dir" ]]; then
